...
(function () {
        ...
        if (typeof loadlive2d === 'function') {
            try {
                ...
                if (retryCount < 3) {
                    ...
                    showMessage('模型加载失败，请刷新页面或切换模型', 3000, true);
                }
            }
        } else {
    ...
    function loadOtherModel() {
        ...
        isLoadingModel = true;
        var modelId = localStorage.getItem('modelId') || 1;
        // 这里可以实现循环切换所有模型ID（如1~N），或自定义模型ID数组
        var nextModelId = parseInt(modelId, 10) + 1;
        if (nextModelId > 120) nextModelId = 1; // 假设有120个模型，根据实际情况调整
        isLoadingModel = false;
        loadModel(nextModelId);
        showMessage('已切换到新模型，ID：' + nextModelId, 3000, true);
    }
