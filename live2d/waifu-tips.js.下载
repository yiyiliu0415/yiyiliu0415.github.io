...
$('.waifu-tool .fui-home').click(function (){
    //window.location = 'https://blog.jixiaob.cn/';
...
(function (){
    ...
    if (window.location.href == SiteIndexUrl) {      // 如果是主页
        ...
        if(document.referrer !== ''){
            ...
            text = '欢迎食用<span style="color:#0099cc;">『' + document.title.split(' - ')[0] + '』</span>~，这里是<span style="color:#FF0000;">赵苦瓜</span>的看板娘desu~';
        }
    }
...
function ifActed() {
    if (!hitokotoInterval) {
        ...
        hitokotoTimer = window.setInterval(showHitokoto, 60000);
    }
}
...
function showHitokoto(){
   // $.getJSON('//api.fghrsh.net/hitokoto/rand/?encode=jsc&uid=3335',function(result){
	    $.getJSON('//hitokoto.jijidown.com/v1/api/hitokoto?maxlength=78',function(result){
...
function loadModel(modelId, modelTexturesId){
	...
	loadlive2d('live2d', '//live2d.jixiaob.cn/get/?id='+modelId+'-'+modelTexturesId, console.log('live2d','模型 '+modelId+'-'+modelTexturesId+' 加载完成'));
    //老API莫名其妙挂了   loadlive2d('live2d', '//api.fghrsh.net/live2d/get/?id='+modelId+'-'+modelTexturesId, console.log('live2d','模型 '+modelId+'-'+modelTexturesId+' 加载完成'));
}
...
function loadRandModel(){
    ...
    $.ajax({
        cache: false,
		url: '//live2d.jixiaob.cn/'+modelTexturesRandMode+'_textures/?id='+modelId+'-'+modelTexturesId,
...
function loadOtherModel(){
    ...
    $.ajax({
        cache: false,
        url: '//live2d.jixiaob.cn/'+modelTexturesRandMode+'/?id='+modelId,      
